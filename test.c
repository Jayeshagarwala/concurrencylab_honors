...
char* test_size0() {
    ...
    for (size_t i = 0; i < SEND_THREAD; i++) {
        mu_assert("test_size0: Testing channel send return failed", data_send[i].out == SUCCESS);
    }

    for (size_t i = 0; i < RECEIVE_THREAD; i++) {
        mu_assert("test_size0: Testing channel receive return value failed", data_rec[i].out == SUCCESS);
        mu_assert("test_size0: Testing channel receive return data failed", string_equal(data_rec[i].data, "Message1"));
    }
...
char* test_non_blocking_size0() {
    ...
    mu_assert("test_non_blocking_size0: Testing channel non blocking buffer", channel_non_blocking_send(channel, "Message_") == CHANNEL_FULL);
    
    void* data = NULL;
    mu_assert("test_non_blocking_size0: Testing channel non blocking buffer", channel_non_blocking_receive(channel, &data) == SUCCESS);
    mu_assert("test_non_blocking_size0: Testing channel non blocking buffer", string_equal(data, "Message"));
    ...
    mu_assert("test_non_blocking_size0: Testing channel non blocking buffer", channel_non_blocking_receive(channel, &out) == CHANNEL_EMPTY);
    mu_assert("test_non_blocking_size0: Testing channel non blocking buffer", channel_non_blocking_send(channel, "Message_1") == SUCCESS);
    
    pthread_join(r_pid, NULL);
    mu_assert("test_non_blocking_size0: Testing channel non blocking buffer", rec_.out == SUCCESS);
    mu_assert("test_non_blocking_size0: Testing channel non blocking buffer", string_equal(rec_.data, "Message_1"));
    ...
    mu_assert("test_non_blocking_size0: Testing channel non blocking buffer", count == 8);
...
char* test_stress_size1() {
    print_test_details(__func__, "Stress Testing for all functions including select using buffered channels (can take some time)");
...
char* test_stress_size0() {
    print_test_details(__func__, "Stress Testing for all functions including select using unbuffered channels (can take some time)");
...
char* test_stress_mixed_size1_size0() {
...
char* test_stress_send_recv_size1() {
    print_test_details(__func__, "Stress Testing for send/recv without select using buffered channels (takes around 10 seconds)");
...
char* test_stress_send_recv_size0() {
    print_test_details(__func__, "Stress Testing for send/recv without select using unbuffered channels (takes around 10 seconds)");
...
char* test_select_and_non_blocking_receive_size1() {
    print_test_details(__func__, "Testing select and non-blocking receive using buffered channels");
    return test_select_and_non_blocking_receive(1);
}

char* test_select_and_non_blocking_receive_size0() {
    print_test_details(__func__, "Testing select and non-blocking receive using unbuffered channels");
    return test_select_and_non_blocking_receive(0);
}

char* test_select_and_non_blocking_send_size1() {
    print_test_details(__func__, "Testing select and non-blocking send using buffered channels");
    return test_select_and_non_blocking_send(1);
}

char* test_select_and_non_blocking_send_size0() {
    print_test_details(__func__, "Testing select and non-blocking send using unbuffered channels");
...
char* test_select_with_select_size1() {
    print_test_details(__func__, "Testing select with select using buffered channels");
    return test_select_with_select(1);
}

char* test_select_with_select_size0 () {
    print_test_details(__func__, "Testing select with select using unbuffered channels");
...
char* test_select_with_same_channel_size1() {
    print_test_details(__func__, "Testing select with same channel using buffered channels");
    return test_select_with_same_channel(1);
}

char* test_select_with_same_channel_size0() {
    print_test_details(__func__, "Testing select with same channel using unbuffered channels");
...
char* test_select_with_send_receive_on_same_channel_size1() {
    print_test_details(__func__, "Testing select with send/recv on same channel using buffered channels");
    return test_select_with_send_receive_on_same_channel(1);
}

char* test_select_with_send_receive_on_same_channel_size0() {
    print_test_details(__func__, "Testing select with send/recv on same channel using unbuffered channels");
...
char* test_select_with_duplicate_channel_size1() {
    print_test_details(__func__, "Testing select with duplicate operations on same channel using buffered channels");
    return test_select_with_duplicate_channel(1);
}

char* test_select_with_duplicate_channel_size0() {
    print_test_details(__func__, "Testing select with duplicate operations on same channel using unbuffered channels");
    return test_select_with_duplicate_channel(0);
}

char* test_select_mixed_size1_size0() {
    ...
    init_object_for_select_api(&args, list, CHANNELS, NULL);
    pthread_create(&pid, NULL, (void *)helper_select, &args);
    mu_assert("test_select_mixed_size1_size0: Receive failed", channel_receive(channel[2], &data) == SUCCESS);
    pthread_join(pid, NULL);
    mu_assert("test_select_mixed_size1_size0: Select failed", args.out == SUCCESS);
    mu_assert("test_select_mixed_size1_size0: Received wrong index", args.index == 2);
    mu_assert("test_select_mixed_size1_size0: Received wrong message", string_equal(data, "Message2"));
    ...
    // To wait sometime before we check select is blocking now
    usleep(10000);
    mu_assert("test_select_mixed_size1_size0: It isn't blocked as expected", args.out == GENERIC_ERROR);
    mu_assert("test_select_mixed_size1_size0: Non-blocking receive failed", channel_non_blocking_receive(channel[2], &data) == SUCCESS);
    pthread_join(pid, NULL);
    mu_assert("test_select_mixed_size1_size0: Select failed", args.out == SUCCESS);
    mu_assert("test_select_mixed_size1_size0: Received wrong index", args.index == 2);
    mu_assert("test_select_mixed_size1_size0: Received wrong message", string_equal(data, "Message2"));
    ...
    init_object_for_select_api(&args, list, CHANNELS, NULL);
    pthread_create(&pid, NULL, (void *)helper_select, &args);
    mu_assert("test_select_mixed_size1_size0: Select failed", channel_select(list_1, 1, &index) == SUCCESS);
    pthread_join(pid, NULL);
    mu_assert("test_select_mixed_size1_size0: Select failed", args.out == SUCCESS);
    mu_assert("test_select_mixed_size1_size0: Received wrong index", args.index == 2);
    mu_assert("test_select_mixed_size1_size0: Received wrong index", index == 0);
    mu_assert("test_select_mixed_size1_size0: Received wrong message", string_equal(list_1[0].data, "Message2"));

    // test receive on buffered channel
    data = NULL;
    init_object_for_select_api(&args, list, CHANNELS, NULL);
    pthread_create(&pid, NULL, (void *)helper_select, &args);
    mu_assert("test_select_mixed_size1_size0: Receive failed", channel_receive(channel[3], &data) == SUCCESS);
    pthread_join(pid, NULL);
    mu_assert("test_select_mixed_size1_size0: Select failed", args.out == SUCCESS);
    mu_assert("test_select_mixed_size1_size0: Received wrong index", args.index == 3);
    mu_assert("test_select_mixed_size1_size0: Received wrong message", string_equal(data, "Message1"));
    ...
    usleep(10000);
    mu_assert("test_select_mixed_size1_size0: It isn't blocked as expected", args.out == GENERIC_ERROR);
    mu_assert("test_select_mixed_size1_size0: Non-blocking receive failed", channel_non_blocking_receive(channel[3], &data) == SUCCESS);
    pthread_join(pid, NULL);
    mu_assert("test_select_mixed_size1_size0: Select failed", args.out == SUCCESS);
    mu_assert("test_select_mixed_size1_size0: Received wrong index", args.index == 3);
    mu_assert("test_select_mixed_size1_size0: Received wrong message", string_equal(data, "Message3"));
    ...
    mu_assert("test_select_mixed_size1_size0: Received wrong index", args.index == 3);
    mu_assert("test_select_mixed_size1_size0: Received wrong index", index == 0);
    mu_assert("test_select_mixed_size1_size0: Received wrong message", string_equal(list_1[0].data, "Message3"));

    // test send on unbuffered channel
    args.select_list[0].data = NULL;
    init_object_for_select_api(&args, list, CHANNELS, NULL);
    pthread_create(&pid, NULL, (void *)helper_select, &args);
    mu_assert("test_select_mixed_size1_size0: Send failed", channel_send(channel[0], "Message4") == SUCCESS);
    pthread_join(pid, NULL);
    mu_assert("test_select_mixed_size1_size0: Select failed", args.out == SUCCESS);
    mu_assert("test_select_mixed_size1_size0: Received wrong index", args.index == 0);
    mu_assert("test_select_mixed_size1_size0: Received wrong message", string_equal(args.select_list[0].data, "Message4"));
    ...
    usleep(10000);
    mu_assert("test_select_mixed_size1_size0: It isn't blocked as expected", args.out == GENERIC_ERROR);
    mu_assert("test_select_mixed_size1_size0: Non-blocking send failed", channel_non_blocking_send(channel[0],"Message5") == SUCCESS);
    pthread_join(pid, NULL);
    mu_assert("test_select_mixed_size1_size0: Select failed", args.out == SUCCESS);
    mu_assert("test_select_mixed_size1_size0: Received wrong index", args.index == 0);
    mu_assert("test_select_mixed_size1_size0: Received wrong message", string_equal(args.select_list[0].data, "Message5"));
    ...
    mu_assert("test_select_mixed_size1_size0: Received wrong index", args.index == 0);
    mu_assert("test_select_mixed_size1_size0: Received wrong index", index == 0);
    mu_assert("test_select_mixed_size1_size0: Received wrong message", string_equal(args.select_list[0].data, "Message6"));

    // test send on buffered channel
    args.select_list[1].data = NULL;
    init_object_for_select_api(&args, list, CHANNELS, NULL);
    pthread_create(&pid, NULL, (void *)helper_select, &args);
    mu_assert("test_select_mixed_size1_size0: Send failed", channel_send(channel[1], "Message7") == SUCCESS);
    pthread_join(pid, NULL);
    mu_assert("test_select_mixed_size1_size0: Select failed", args.out == SUCCESS);
    mu_assert("test_select_mixed_size1_size0: Received wrong index", args.index == 1);
    mu_assert("test_select_mixed_size1_size0: Received wrong message", string_equal(args.select_list[1].data, "Message7"));
    ...
    usleep(10000);
    mu_assert("test_select_mixed_size1_size0: It isn't blocked as expected", args.out == GENERIC_ERROR);
    mu_assert("test_select_mixed_size1_size0: Non-blocking send failed", channel_non_blocking_send(channel[1], "Message8") == SUCCESS);
    pthread_join(pid, NULL);
    mu_assert("test_select_mixed_size1_size0: Select failed", args.out == SUCCESS);
    mu_assert("test_select_mixed_size1_size0: Received wrong index", args.index == 1);
    mu_assert("test_select_mixed_size1_size0: Received wrong message", string_equal(args.select_list[1].data, "Message8"));
    ...
    mu_assert("test_select_mixed_size1_size0: Received wrong index", args.index == 1);
    mu_assert("test_select_mixed_size1_size0: Received wrong index", index == 0);
    mu_assert("test_select_mixed_size1_size0: Received wrong message", string_equal(args.select_list[1].data, "Message9"));
...
test_t tests[] = {{"test_initialization", test_initialization},
                  ...
                  {"test_stress_send_recv_size1", test_stress_send_recv_size1},
                  ...
                  {"test_stress_size1", test_stress_size1},
                  ...
                  {"test_size0", test_size0},
                  {"test_non_blocking_size0", test_non_blocking_size0},
                  {"test_stress_send_recv_size0", test_stress_send_recv_size0},
                  {"test_select_and_non_blocking_send_size0", test_select_and_non_blocking_send_size0},
                  {"test_select_and_non_blocking_receive_size0", test_select_and_non_blocking_receive_size0},
                  {"test_select_with_select_size0", test_select_with_select_size0},
                  {"test_select_with_same_channel_size0", test_select_with_same_channel_size0},
                  {"test_select_with_send_receive_on_same_channel_size0", test_select_with_send_receive_on_same_channel_size0},
                  {"test_select_with_duplicate_channel_size0", test_select_with_duplicate_channel_size0},
                  {"test_select_mixed_size1_size0", test_select_mixed_size1_size0},
                  {"test_stress_size0", test_stress_size0},
                  {"test_stress_mixed_size1_size0", test_stress_mixed_size1_size0},
};
