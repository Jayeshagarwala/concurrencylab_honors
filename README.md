...
In the honors option, you will add support for unbuffered channels where the channel size is 0. In this case, the buffer is unused, and there is no buffer space for storing messages. This means that senders will need to wait until there is a receiver and receivers will need to wait until there is a sender. Only when there is a pair of senders and receivers are both threads allowed to continue running. This also applies to select where the select thread needs to wait until there is another send/receive/select thread that can complete the message transfer. Thus, the synchronization is very different from a channel of size 1, and implementing this feature may require a complete redesign. You are recommended to spend time thinking critically about how to design the synchronization to support both unbuffered and buffered channels. If you have any questions about this part of the project, please contact only the course instructor for clarification.
